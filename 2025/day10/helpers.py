...
def a_star(
    ...
    while len(open):
        est, cost, loc  = heapq.heappop(open)
        if i % 10000 == 0:
            print(f'best cand has est {est}, cost {cost}, state {loc}')
        if loc == end_loc:
            return cost, None #construct_path(came_from, start_loc, end_loc)
        for next_cost, next_loc in generate_fn(context, cost, loc, end_loc):
